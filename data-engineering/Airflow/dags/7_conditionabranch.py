...
def branch_dag():
    ...
    @task.python
    def transform_task_db(**kwargs:object)-> None:
        ti = kwargs['ti']
    ...
    @task.python
    def transform_task_s3(**kwargs:object)-> None:
        ti = kwargs['ti']
        s3_transformed_data = ti.xcom_pull(task_ids = 'extract_task')["s3_extracted_data"]
        print(f"Transforming S3 data: {s3_transformed_data}")
        transformed_s3_data = [i*1000 for i in s3_transformed_data]
    ...
    def decider_task(**kwargs):
        ...
        if weekend_flag == "true":
            return 'no_load_task' #always return task id
        else:
            return "load_task"
    ...
    def load_task(**kwargs):
        ...
        return f"echo '{api_data}, {db_data}, {s3_data}'"
